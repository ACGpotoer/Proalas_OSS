...
def func(ev: threading.Event):
    ...
    if use_dap and not ssl:
        ...
        logger.attr("DapShell", True)
        # Electron only reads stderr for "Application startup complete"
        sys.stderr.write("[ProAlas] DAP shell enabled, port=%s\n" % port)
        sys.stderr.flush()
        if State.electron:
            from module.logger import console_hdlr

            logger.removeHandler(console_hdlr)
        run_dap_shell(host, port)
        return
    # ===== end ProAlas OSS =====
